...
| --- | --- | --- |
| **shell** | Hyprland + Walker + Ghostty feels like Omarchy at first login | Enables `programs.hyprland` (UWSM), Ghostty, Walker, Waybar (workspaces, clock/calendar, wifi, bluetooth, audio, cpu, battery, tray), Elephant (Walker 2.x backend, including menus/clipboard/calc/files/symbols), **hyprlock + hypridle** (lock on idle), **mako** (notifications), **hyprsunset** (night light) + **swayosd** (volume/brightness/caps OSD), and **SDDM + Plymouth** (greeter / unlock art). Home Manager writes the Hyprland/Ghostty/lock/OSD/Waybar baseline, Walker config + GTK CSS, and keybinds. |
...
That `home-manager.users.youruser` block is **not optional**. The NixOS module installs Hyprland and the SDDM session. Home Manager writes `~/.config/hypr/hyprland.conf` (Super+Return Ghostty, Super+Space Walker, Waybar, mako). Skip the user wiring and Hyprland drops an autogenerated stub (`SUPER+Q` → kitty) the first time you log in. See [Home Manager writes the session](#home-manager-writes-the-session).
...
### Home Manager writes the session

`programs.omarchy.enable` on NixOS is not enough for an Omarchy session. Two modules, one job each:

| Module | What it does | What it does **not** do |
| --- | --- | --- |
| `omarchy-nix.nixosModules.default` | Hyprland + UWSM, SDDM, Plymouth, packages on PATH | Write `~/.config/hypr/hyprland.conf` |
| `omarchy-nix.homeManagerModules.default` | Keybinds, Waybar, Walker/Elephant units, Ghostty, mako, theme seed | Install the display-manager session |

If the user module is missing, Hyprland has no config and writes the stock stub at `~/.config/hypr/hyprland.conf` (`SUPER+Q` → kitty, yellow “autogenerated config” banner). Walker, Waybar, and mako stay down — they are started from that managed file and from `graphical-session.target`, not from the NixOS module.

**Minimal NixOS + Home Manager wiring** (copy this; the flake example above is the same shape):
...
`youruser` must be a real `users.users.youruser`. `stateVersion` is required by Home Manager.

What the modules do so this is hard to get wrong:

- The NixOS module **asserts** when the shell pillar is on and no Home Manager user has `wayland.windowManager.hyprland.enable`. Rebuild fails instead of giving you kitty binds.
- When `home-manager.nixosModules.home-manager` is imported, the NixOS module also injects `homeManagerModules.default` via `home-manager.sharedModules`. The per-user `imports` line is still the documented path; the inject covers a forgotten import.
- Home Manager pins `wayland.windowManager.hyprland.configType = "hyprlang"` and `systemd.enable = !programs.omarchy.shell.withUWSM`.
- When `shell.withUWSM` is on (the default), every app keybind is `uwsm-app -- …`. Waybar, Walker, Elephant, hypridle, hyprsunset, swayosd, and theme-apply are systemd user units on `graphical-session.target`; mako is Hyprland `exec-once`.
- Home Manager writes `~/.config/hypr/hyprland.conf`. Log out and back in, or `hyprctl reload`, after the first switch.
- Waybar is a user systemd unit with workspaces, clock, wifi, bluetooth, pulseaudio, cpu, battery, and tray. Walker and Elephant already were.

Standalone Home Manager (no NixOS module):
...
`osConfig` is empty in that mode, so `programs.omarchy.enable` is not inherited — set it. PAM for hyprlock and the swayosd libinput backend still have to be set on the host.
...
App chords are Omarchy’s `plain-bindings.conf` (Shift variants). Super+B / Super+N / Super+F are **not** apps — Super+F is fullscreen (`tiling-v2.conf`). With `shell.withUWSM` (default), app execs go through `uwsm-app --`. Not ported: `omarchy-menu`, window-pop, lid/monitor helpers, dictation, and the rest of the utilities that are not in this flake.
...
| Side | Modules |
| --- | --- |
| Left | NixOS logo (opens Walker) + Hyprland workspaces 1–5 |
| Center | Clock (`%A %H:%M`; click toggles date; tooltip is a month calendar) |
| Right | Tray expander, bluetooth, wifi, pulseaudio, cpu, battery |

Clicks open Walker, `nm-connection-editor`, `blueman-manager`, `pavucontrol`, and `btop` in Ghostty. Colors follow `omarchy-theme-set` via `current/waybar.css`.
...
Both are on by default with the shell pillar (`programs.omarchy.shell.greeter`). Autologin is **off**. The session SDDM starts is `hyprland-uwsm` when `shell.withUWSM` is on (the default; same UWSM wrap Omarchy’s `omarchy.desktop` uses). Set `shell.withUWSM = false` and the default session is bare `hyprland`.
...
1. **Shell**
   ...
   - [x] Waybar (Omarchy’s actual status bar; Walker is the launcher) with workspaces, clock/calendar, wifi, bluetooth, audio, cpu, battery, tray

...
- Home Manager pins `wayland.windowManager.hyprland.configType = "hyprlang"` (HM 26.05+ would otherwise write `hyprland.lua`) and `systemd.enable = !programs.omarchy.shell.withUWSM` (UWSM already owns `graphical-session.target`).
- When `shell.withUWSM` is on (the default), every app keybind is `uwsm-app -- …` (terminal, browser, files, editor, Walker, screenshots). Bare `exec` under `hyprland-uwsm` is a silent no-op. Waybar, Walker, Elephant, hypridle, hyprsunset, swayosd, and theme-apply are systemd user units on `graphical-session.target`; mako is Hyprland `exec-once` (also `uwsm-app` when UWSM is on).
- A leftover autogenerated `~/.config/hypr/hyprland.conf` is **replaced** on the next switch (`force`). An autogenerated `hyprland.lua` is removed so it cannot win over the managed `.conf`. Log out and back in, or `hyprctl reload`.
- Waybar is a user systemd unit (`programs.waybar.systemd.enable`). Walker and Elephant already were.

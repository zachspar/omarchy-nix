...
in
{
  config = lib.mkIf (cfg.enable && cfg.shell.enable) {
    ...
    # Hyprland 0.55+ loads hyprland.lua in preference to hyprland.conf.
    # Always drop a leftover lua file (autogen or otherwise) so the managed
    # hyprlang session is what starts. Runs after writeBoundary so a failed
    # earlier activation step cannot leave you with neither file.
    home.activation.omarchyRemoveHyprlandLua = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
      rm -f "${config.xdg.configHome}/hypr/hyprland.lua"
    '';

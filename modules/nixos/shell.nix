  ...
  config = lib.mkMerge (
    [
      (lib.mkIf (cfg.enable && cfg.shell.enable) {
        ...
        # Waybar bluetooth module + blueman-manager on click. Hosts can
        # still set hardware.bluetooth.enable = false.
    ...
    ++ lib.optionals (options ? home-manager) [
      (lib.mkIf (cfg.enable && cfg.shell.enable) {
        ...
        # First login / Plasma leftover files (GTK, user-dirs) otherwise
        # abort HM activation and leave Hyprland on the autogenerated lua
        # stub. Files we own are also force-replaced; this is the net for
        # anything else already in $HOME.
